...
$user_id = $_SESSION['id'];
$pid = $_GET['pid'];
$wc = 1; // Assuming this is a constant value
$quantity = 0; // Assuming this is a constant value

// Check if the combination of user_id and pid already exists in the wishcart table
$query = "SELECT * FROM wishcart WHERE user_id = $user_id AND pid = $pid AND wc = 1";
$result = mysqli_query($conn, $query);

if (mysqli_num_rows($result) > 0) {
    // Product already exists in wishlist, display a message
    $_SESSION['no'] = "This product is already in your wishlist.";
...
exit();
    ...
    if (mysqli_query($conn, $insert_query)) {
        ...
        $update_query = "UPDATE products SET wishlist = wishlist + 1 WHERE pid = $pid";
